...
void FBlueprintEditor::CreateDefaultCommands()
{
	...
	ToolkitCommands->MapAction(FBlueprintEditorCommands::Get().ConvertSelectedToISM,
		FExecuteAction::CreateSP(this, &FBlueprintEditor::ConvertToISM_OnClicked),
		FCanExecuteAction::CreateSP(this, &FBlueprintEditor::IsInAScriptingMode)
	);
...
void FBlueprintEditor::ConvertToISM_OnClicked()
{
	TArray<FSCSEditorTreeNodePtrType> SelectedComponents = GetSelectedSCSEditorTreeNodes();
	if (SelectedComponents.Num())
	{
		ConvertSelectedComponentsToISMs(SelectedComponents);
	}
	else
	{
		UE_LOG(LogTemp, Warning, TEXT("[Debug] Cannot convert selected components to instanced static mesh, because current focused blueprint graph is invalid."));
	}
}
...
// -- Begin mod section --
struct FISMConvertEntry
{
	UStaticMesh* TargetStaticMesh = nullptr;
	UMaterialInterface* TargetMaterial = nullptr;

	TArray<FSCSEditorTreeNodePtrType> TargetNodeArr;
};

TSharedPtr<FSCSEditorTreeNode> GetComponentNodeParent(TSharedPtr<FSCSEditorTreeNode> Node)
{
	TSharedPtr<FSCSEditorTreeNode> ParentNode = Node->GetParent();
	if (ParentNode.IsValid() && ParentNode->IsRootComponent())
	{
		return ParentNode;
	}
	else
	{
		return GetComponentNodeParent(ParentNode);
	}
}

void FBlueprintEditor::ConvertSelectedComponentsToISMs(TArray<FSCSEditorTreeNodePtrType> SelectedNodes)
{
	// Convert selected components to instanced static mesh component

	auto CanBeInstanced = [](const UStaticMeshComponent* InStaticMeshComponent) -> bool
	{
		// Dont handle negative scale for ISM
		const FVector& Scale = InStaticMeshComponent->GetRelativeScale3D();
		bool bIsScaleValid = Scale.GetMin()> 0.0f;

		// ...and ignore shared root for now
		bool bIsRoot = InStaticMeshComponent->GetName() == TEXT("SharedRoot");

		return bIsScaleValid && !bIsRoot;
	};

	// Prepare instanced data
	TArray<FSCSEditorTreeNodePtrType> CandidateArr;   // Candidates for all UStaticMeshComponents that want to instanced
	TSharedPtr<FSCSEditorTreeNode> RootComponentNode;

	for (FSCSEditorTreeNodePtrType Node: SelectedNodes)
	{
		UActorComponent* Component = Node->GetComponentTemplate();
		if(UStaticMeshComponent* SelectedStaticMeshComponent = Cast<UStaticMeshComponent>(Component))
		{
			if (!RootComponentNode)
			{
				RootComponentNode = GetComponentNodeParent(Node);
			}

			if (CanBeInstanced(SelectedStaticMeshComponent))
			{
				// Added to candidate list
				CandidateArr.Add(Node);
			}
		}
	}

	// Collect data to instanced
	TMap<const FString, FISMConvertEntry> PendingInstancedMap;

	for (FSCSEditorTreeNodePtrType CandidateNode: CandidateArr)
	{
		UStaticMeshComponent* StaticMeshComponent = Cast<UStaticMeshComponent>(CandidateNode->GetComponentTemplate());
		UStaticMesh* StaticMesh = StaticMeshComponent->GetStaticMesh();
		UMaterialInterface* Material = StaticMeshComponent->GetMaterial(0);	// Fixed to element 0, for only one material slot in Afterimage

		if (StaticMesh && Material)
		{
			const FString& Name = StaticMesh->GetName(); // As primary key

			if (!PendingInstancedMap.Contains(Name))
			{
				FISMConvertEntry Entry;
				Entry.TargetStaticMesh = StaticMesh;
				Entry.TargetMaterial = Material;
				Entry.TargetNodeArr.Add(CandidateNode);

				PendingInstancedMap.Add(Name, Entry);
			}
			else
			{
				FISMConvertEntry& ExistEntry = PendingInstancedMap[Name];
				if (Material == ExistEntry.TargetMaterial)
				{
					PendingInstancedMap[Name].TargetNodeArr.Add(CandidateNode);
				}
			}
		}
	}
	...
	// Construct ISM
	ensure(RootComponentNode);
	USceneComponent* RootComponent = Cast<USceneComponent>(RootComponentNode->GetComponentTemplate());

	if (RootComponent)
	{
		RootComponent->SetMobility(EComponentMobility::Static);
		RootComponent = RootComponent->GetAttachmentRoot();
	}
	else
	{
		UE_LOG(LogTemp, Warning, TEXT("[Debug] Failed to get attachment root for component %s."), *(RootComponent->GetName()));
	}

	UBlueprint* Blueprint = RootComponentNode->GetBlueprint();
	USimpleConstructionScript* SCS = Blueprint->SimpleConstructionScript;

	SSCSEditor::FAddNewComponentParams ComponentCreateParam;
	ComponentCreateParam.bConformTransformToParent = true;
	ComponentCreateParam.bSetFocusToNewItem = false;
	ComponentCreateParam.bSkipMarkBlueprintModified = false;

	for (auto Pair :PendingInstancedMap)
	{
		FISMConvertEntry& Entry = Pair.Value;
		FName ISMName = FName(Pair.Key);

		if (Entry.TargetNodeArr.Num() > 1)
		{
			UActorComponent* BaseComp = SCSEditor->AddNewComponent(UInstancedStaticMeshComponent::StaticClass(), nullptr, ComponentCreateParam);
			UInstancedStaticMeshComponent* InstancedStaticMeshComp = Cast<UInstancedStaticMeshComponent>(BaseComp);
			InstancedStaticMeshComp->CreationMethod = EComponentCreationMethod::SimpleConstructionScript;
			InstancedStaticMeshComp->SetupAttachment(RootComponent);
			InstancedStaticMeshComp->SetStaticMesh(Entry.TargetStaticMesh);
			InstancedStaticMeshComp->SetMaterial(0, Entry.TargetMaterial);
			InstancedStaticMeshComp->SetGenerateOverlapEvents(false);
			InstancedStaticMeshComp->SetMobility(EComponentMobility::Static);	// all static?

			// No interface is expose, so use this method to avoid modify SSCSEditor..
			for (USCS_Node* Node: SCS->GetAllNodes())
			{
				if (Node->ComponentTemplate == BaseComp)
				{
					Node->SetVariableName(ISMName);
					break;
				}
			}

			// Update instance and remove old componet
			for (FSCSEditorTreeNodePtrType TargetNode : Entry.TargetNodeArr)
			{
				UStaticMeshComponent* TargetComp = Cast<UStaticMeshComponent>(TargetNode->GetComponentTemplate());

				// TODO: add support for multi-child component

				// Used world space transform to avoid bugs
				InstancedStaticMeshComp->AddInstance(TargetComp->GetRelativeTransform());

				if (TargetNode->CanDelete())
				{
					SCSEditor->RemoveComponentNode(TargetNode);
				}
				else
				{
					UE_LOG(LogTemp, Warning, TEXT("[Debug] Target node %s can not be removed."), *(TargetNode->GetDisplayName().ToString()));
				}
			}

			InstancedStaticMeshComp->MarkRenderStateDirty();
			InstancedStaticMeshComp->MarkRenderTransformDirty();
		}
	}
