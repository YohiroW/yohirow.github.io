...
根据官方文档中所说[^VSM]，Nanite几何体始终渲染到虚拟阴影贴图，是性能最高的选项，可提供最高质量。然而这个游戏中并没有默认开启 VSM。手动开启 VSM 后，发现渲染线程时间和 GPU 时间均获得了 2 ms 左右的提升。
...
VSM 的 PhysicalPagePool 在引擎中默认的分辨率是 16384 x 4096，RoboCop 里使用的是 16384 x 2048，每个 Tile 的大小是 128 x 128。
